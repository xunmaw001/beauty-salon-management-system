    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="套餐内容" prop="taocanneirong">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="套餐内容"
                  v-model="ruleForm.taocanneirong" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.taocanneirong" label="套餐内容" prop="taocanneirong">
                    <span>{{ruleForm.taocanneirong}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.keyueshijian = true;
          ...
          if(o=='taocanneirong'){
            this.ruleForm.taocanneirong = obj[o];
	    this.ro.taocanneirong = true;
            continue;
          }
	...
	this.ruleForm.taocanxiangqing = this.ruleForm.taocanxiangqing.replace(reg,'../../../ssmje091/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `meirongtaocan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.meirongtaocanCrossAddOrUpdateFlag = false;
						 this.parent.search();
