                ...
                <el-table-column
                 ...
                 :header-align="contents.tableAlign"
                  ...
                  <template slot-scope="scope">
                    ...
                    <el-button v-if="scope.row.ispay!='已支付' && isAuth('yuangonggongzi','支付') " type="text" icon="el-icon-edit" size="small" @click="payHandler(scope.row)">支付</el-button>
                  </template>
                </el-table-column>
...
export default {
  ...
  methods: {
    ...
    chartDialog() {
      ...
      this.$nextTick(()=>{
        // nianyuefen nianyuefen
        //  jine
        // xcolumn ycolumn nianyuefen jine
        var jineChart = this.$echarts.init(document.getElementById("jineChart"),'macarons');
        this.$http({
            url: `yuangonggongzi/value/nianyuefen/jine`,
            method: "get"
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].nianyuefen);
                    yAxis.push(parseFloat((res[i].total)).toFixed(2));
                    pArray.push({
                    value: parseFloat((res[i].total)),
                    name: res[i].nianyuefen
                    })
                    var option = {};
                    option = {
                        tooltip: {
